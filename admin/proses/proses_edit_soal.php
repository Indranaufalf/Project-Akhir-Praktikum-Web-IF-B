...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $stmt = $db->prepare("UPDATE soal SET 
                         kelas = ?,
                         mata_pelajaran = ?,
                         soal = ?,
                         paket_soal = ?,
                         ...
                         bobot = ?
                         WHERE id = ?");
    $stmt->bind_param("ississssssii", $kelas, $mata_pelajaran, $soal, $paket_soal, 
                      $pilihan_a, $pilihan_b, $pilihan_c, $pilihan_d, 
                      $jawaban_benar, $pembahasan, $bobot, $id_soal);
    
    if($stmt->execute()){
        $_SESSION['pesan'] = "Soal berhasil diupdate!";
    } else {
        $_SESSION['pesan'] = "Gagal mengupdate soal!";
    }
